    ...
    switch (settingName) {
      ...
      case 'showFps':
        ...
        if (value && !this.this.settings.fpsDiv) {
          throw new Error(`fpsDiv must be configured before enabling 'showFps'`);
        }
        break;
      case 'pointDensity':
        if (value < 0) {
    ...
    window.addEventListener(
      ...
      function (e) {
        ...
        if (pointCountDiff > 0) {
          ...
          this.addPoints(-pointCountDiff, this.settings.pointColor, this.settings.pointSize);
        }
      }.bind(this)
    );
  }
...
export default {
  ...
  mounted() {
    ...
    constellation = new Constellation(configuration);
    constellation.init();
    constellation.start();
  }
};
